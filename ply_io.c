...
typedef int bool;
void get_binary_item(FILE *, int, int *, unsigned int *, double *, bool is_big_endian);
...
void binary_get_element(PlyFile *plyfile, char *elem_ptr)
{
  ...
  for (j = 0; j < elem->nprops; j++) {
    ...
    if (prop->is_list == PLY_LIST || prop->is_list == PLY_LIST_FIXED) {          /* list */

	/* get and store the number of items in the list */
	get_binary_item (fp, prop->count_external,
			    &int_val, &uint_val, &double_val, plyfile->file_type == PLY_BINARY_BE);
	if (store_it && prop->is_list == PLY_LIST) {
	  item = elem_data + prop->count_offset;
	  store_item(item, prop->count_internal, int_val, uint_val, double_val);
	}
	...
	if (prop->is_list == PLY_LIST_FIXED && list_count != prop->count_offset) { plyfile->error = 1; return; }
	...
	if (list_count == 0) {
	  ...
	  if (store_it) {
	    if (prop->is_list == PLY_LIST_FIXED)
	    {
		    item_ptr = (char*)(elem_data + prop->offset);
	    }
	    else
...
void reverse(uint8_t* a, int bytes)
{
	while (bytes > 0)
	{
		uint8_t w = a[0];
...
)
{
  ...
  switch (type) {
	...
	*double_val = *uint_val;
    ...
    case Int16:
	fread (ptr, 2, 1, fp);
	if (do_reverse) reverse((uint8_t*)ptr, 2);
	*int_val = *((short int *) ptr);
	*uint_val = *int_val;
	*double_val = *int_val;
	break;
	...
	*double_val = *uint_val;
    ...
    case Int32:
	fread (ptr, 4, 1, fp);
	if (do_reverse) reverse((uint8_t*)ptr, 4);
	*int_val = *((int *) ptr);
	*uint_val = *int_val;
	*double_val = *int_val;
	break;
	...
	*uint_val = *double_val;
    ...
    case Float64:
	fread (ptr, 8, 1, fp);
	if (do_reverse) reverse((uint8_t*)ptr, 8);
	*double_val = *((double *) ptr);
	*int_val = *double_val;
	*uint_val = *double_val;
	break;
